...
E1. To ćwiczenie wymagapakietu**z22**, umożliwiającego dostęp do danych o rozdzielczości 100 m.
Zainstaluj go za pomocą polecenia \``remotes::install_github("JsLth/z22")`\`.
Załaduj dane dotyczące ludności o rozdzielczości komórek 100 m,korzystając z adresu`z22::z22_data("population", res = "100m", year = 2022)`.
Zagreguj je do rozdzielczości komórek 1 km,korzystając z adresu`terra::aggregate()`oraz opcji \``fun = sum`\`, a następnie porównaj wynik z danymi o rozdzielczości 1 km dostępnymi pod adresem`census_de`.
Należy pamiętać, że plik danych o rozdzielczości 100 m jest znacznie większy i jego pobranie może zająć trochę czasu.
...
inh_fine_agg = terra::aggregate(
  inh_fine, fact = terra::res(inh_coarse)[1] / terra::res(inh_fine)[1], fun = sum, na.rm = TRUE)
...
E2. Załóżmy, że nasz sklep rowerowy sprzedawał głównie rowery elektryczne osobom starszym.
Zmień odpowiednio raster wieku, powtórz pozostałe analizy i porównaj zmiany z naszym pierwotnym wynikiem.
