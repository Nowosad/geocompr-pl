...
E1. Oblicz globalne nasłonecznienie dla obszaru`system.file("raster/dem.tif", package = "spDataLarge")`dla dnia 21 marca o godz. 11:00, korzystając z modułu`r.sun`GRASS GIS poprzez**qgisprocess**.
...
E2. Oblicz powierzchnię zlewni\\index{catchment area}i nachylenie zlewni`system.file("raster/dem.tif", package = "spDataLarge")`,korzystając z**Rsagacmd**.
...
E3. Kontynuuj pracę nadobiektem`ndvi_segments`utworzonym w sekcji SAGA.
Wyodrębnij średnie wartości NDVI zrastra`ndvi`i pogrupuj je w sześć klastrów za pomocą`kmeans()`.
Wizualizuj wyniki.
...
E4. Dołączplik`data(random_points, package = "spDataLarge")`i wczytaj plik`system.file("raster/dem.tif", package = "spDataLarge")`do programu R.
Wybierz losowo punkt z pliku`random_points`i znajdź wszystkiepiksele`dem`, które są widoczne z tego punktu (wskazówka: viewshed\\index{viewshed}można obliczyć za pomocą programu GRASS GIS).
Wizualizuj wynik.
Na przykład, narysuj hillshade\\index{hillshade}, cyfrowy model terenu\\index{digital elevation model},wynikviewshed\\index{viewshed}oraz punkt.
Dodatkowo, wypróbuj`mapview`.
...
E5. Użyj`gdalinfo`poprzez wywołanie systemowe dlaplikurastrowego\\index{raster}przechowywanego na wybranym dysku.
...
E6. Użyj`gdalwarp`, aby zmniejszyć rozdzielczość pliku rastrowego (na przykład, jeśli rozdzielczość wynosi 0,5, zmień ją na 1). Uwaga:w tym ćwiczeniu zostaną użyte flagi`-tr`i`-r`.
...
E7. Wyszukaj wszystkie autostrady w Kalifornii z bazydanychPostgreSQL/PostGIS\\index{PostGIS}znajdującej się wchmurzeQGIS\\index{QGIS}przedstawionej w tym rozdziale.
...
E8.Raster„`ndvi.tif`”(`system.file("raster/ndvi.tif", package = "spDataLarge")`) zawiera wskaźnik NDVI obliczony dla obszaru badań Mongón na podstawie danych Landsat z 22 września 2000 r.
Użyj**rstac**,**gdalcubes**i**terra**,aby pobrać obrazy Sentinel-2 dla tego samego obszaru z okresu
od01\.08.2020 r. do 31.10.2020 r., obliczyć jego NDVI, a następnie porównać go z wynikami z`ndvi.tif`.
