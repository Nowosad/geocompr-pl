Rozwiązania zakładają, że dołączone są następujące pakiety (inne pakiety zostaną dołączone w razie potrzeby):
...
E1. Uruchom NMDS\\index{NMDS}, korzystając z danych procentowych macierzy społeczności.
Podaj wartość stresu i porównaj ją z wartością stresu uzyskaną z NMDS przy użyciu danych dotyczących obecności/nieobecności.
Co może wyjaśniać zaobserwowaną różnicę?
...
E2. Oblicz wszystkie rastrowe zmienne prognostyczne\\index{raster}, które wykorzystaliśmy w tym rozdziale (nachylenie zlewni, powierzchnia zlewni) i umieść je wobiekcie`SpatRaster`.
Dodajdo niego`dem`i`ndvi`.
Następnie oblicz profil i krzywiznę styczną i dodaj je jako dodatkowe rastrowe zmienne prognostyczne (wskazówka:`grass7:r.slope.aspect`).
Na koniec skonstruuj macierz odpowiedzi-predyktora.
Wyniki pierwszejosiNMDS\\index{NMDS}(które były wynikiem zastosowania macierzy obecności-nieobecności społeczności) obrócone zgodnie z wysokością nad poziomem morza reprezentują zmienną odpowiedzi i powinny zostać połączone z`random_points`(użyj połączenia wewnętrznego).
Aby uzupełnić macierz odpowiedzi-predyktora, wyodrębnij wartości obiektu rastra predyktora środowiskowego do`random_points`.
...
E3. Pobierz RMSE z redukcją błędu systematycznego dla lasu losowego\\index{random forest}i modelu liniowego przy użyciu przestrzennej walidacji krzyżowej\\index{cross-validation!spatial CV}.
Modelowanie lasu losowego powinno obejmować oszacowanie optymalnychkombinacjihiperparametrów\\index{hyperparameter}(wyszukiwanie losowe z 50 iteracjami) w wewnętrznej pętli dostrajania.
Zrównoleglaj\\index{parallelization}poziom dostrajania.
Zgłoś średnią RMSE\\index{RMSE}i użyj wykresu pudełkowego, aby zwizualizować wszystkie pobrane wartości RMSE.
Należy pamiętać, że to ćwiczenie najlepiej rozwiązać za pomocą funkcji mlr3`benchmark_grid()`i`benchmark()`(więcej informacjimożna znaleźćnastronie<https://mlr3book.mlr-org.com/perf-eval-cmp.html#benchmarking>).
